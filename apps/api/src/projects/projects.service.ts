 ...
 * Implements DRAFT → UNDER_REVIEW → LIVE transitions (review approval
 * lives in the admin module; here we only allow the creator to submit).
 *
 * Money is stored as BigInt halalas; we serialize via toPublic().
 */
@Injectable()
export class ProjectsService {
  ...
  async create(creatorId: string, dto: CreateProjectDto): Promise<Project> {
    // The deadline is set when the project goes LIVE, not at create-time —
    // store a provisional date based on durationDays from "now".
    ...
    return this.prisma.project.create({
      data: {
        ...
        platformPartner: dto.platformPartner ? (dto.platformPartner as unknown as Prisma.InputJsonValue) : Prisma.JsonNull,
      },
    });
  }
  ...
  /** Creator submits for admin review. */
  async submitForReview(creatorId: string, projectId: string): Promise<Project> {
    ...
    // Sanity validations before submitting
  ...
  /** Admin-only — approves a project and starts the funding clock. */
  ...
  async list(q: ListProjectsQueryDto): Promise<{ items: Project[]; nextCursor: string | null }> {
    ...
    else where.status = { in: [ProjectStatus.LIVE, ProjectStatus.SUCCESSFUL, ProjectStatus.FUNDED] };
  ...
  toPublic(p: Project & { rewardTiers?: Array<Record<string, unknown>> }): Record<string, unknown> {
    return {
      id: p.id,
      titleAr: p.titleAr,
