...
export class MilestonesController {
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Submit evidence for a milestone (creator only)' })
  ...
  // -- admin only (Phase 4 will add a role guard) -----------------------------
  ...
  @Get('transparency')
  @ApiOperation({ summary: 'Live Transparency Dashboard payload (public)' })
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Add a spend log entry (creator only)' })
  async addSpend(
    @CurrentUser() jwt: JwtPayload,
    @Param('projectId', new ParseUUIDPipe()) projectId: string,
