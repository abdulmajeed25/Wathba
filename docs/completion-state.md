...
- ~~**Post-deploy step:** apply `apps/api/prisma/_raw/searchVector.sql`~~ — **RESOLVED and REVERSED.** Migration `0062_arabic_search` moved the tsvector column into the chain and the sidecar file is deleted; `prisma migrate deploy` is now sufficient. Applying the old sidecar on top of 0062 is actively harmful: it rebuilds `searchVector` with the diacritics-only function, silently switching off hamza / ta-marbuta / alef-maksura / Arabic-Indic-digit folding. Nothing errors — searches just quietly return fewer results.
